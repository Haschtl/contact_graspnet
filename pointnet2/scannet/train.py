...
TRAIN_DATASET = scannet_dataset.ScannetDataset(
    root=DATA_PATH, npoints=NUM_POINT, split='train')
TEST_DATASET = scannet_dataset.ScannetDataset(
    root=DATA_PATH, npoints=NUM_POINT, split='test')
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 5 == 0:
                acc = eval_one_epoch(session, ops, test_writer)
                acc = eval_whole_scene_one_epoch(session, ops, test_writer)
            if acc > best_acc:
                best_acc = acc
                save_path = saver.save(session, os.path.join(
                    LOG_DIR, "best_model_epoch_%03d.ckpt" % (epoch)))
                log_string("Model saved in file: %s" % save_path)

            # Save the variables to disk.
            if epoch % 10 == 0:
                save_path = saver.save(
                    session, os.path.join(LOG_DIR, "model.ckpt"))
                log_string("Model saved in file: %s" % save_path)
...
def eval_whole_scene_one_epoch(session, ops, test_writer):
    """ ops: dict mapping from string to tf ops """
    global EPOCH_CNT
    is_training = False
    ...
    for batch_idx in range(num_batches):
        if not is_continue_batch:
            ...
            batch_data_tmp, batch_label_tmp, batch_smpw_tmp = TEST_DATASET_WHOLE_SCENE[
                batch_idx]
