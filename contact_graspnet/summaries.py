...
def top_grasp_acc_summaries(ops, threshold=[0.62, 0.65, 0.68]):
    """
    ...
    Keyword Arguments:
        threshold {list} -- thresholds to report (default: {[0.62,0.65,0.68]})

    Returns:
        [tf.variable] -- summary update ops
    """

    binary_seg = tf.reshape(ops['binary_seg_pred'], [
                            ops['binary_seg_pred'].shape[0], ops['binary_seg_pred'].shape[1]])
    ...
    acc_update_ops = [0] * len(threshold)
    accs = [0] * len(threshold)
    for i, thr in enumerate(threshold):
        accs[i], acc_update_ops[i] = tf.metrics.accuracy(
            top_labels, tf.greater(max_confs, tf.constant(thr)))
        tf.summary.scalar('Top Grasp Accuracy Thres %s' % thr, accs[i])

    return acc_update_ops


def build_summary_ops(ops, session, global_config):
    """
    ...
    tf.summary.scalar('offset_loss_x%s' %
                      global_config['OPTIMIZER']['offset_loss_weight'], ops['offset_loss'])

    tf.summary.histogram('labels_grasp_suc', ops['grasp_suc_labels_pc'])
    tf.summary.histogram('predictions_grasp_suc', ops['binary_seg_pred'])
    ...
    if global_config['MODEL']['bin_offsets']:
        ...
        tf.summary.histogram('offset_predictions_binned',
                             ops['offset_pred_idcs_pc'])
        tf.summary.histogram('offset_labels_binned',
                             ops['offset_label_idcs_pc'])
        tf.summary.scalar('offset_bin_classification_accuracy', tf.reduce_mean(tf.cast(
            tf.equal(ops['offset_pred_idcs_pc'], ops['offset_label_idcs_pc']), tf.float32)))

        bin_mean_predictions = ops['offset_bin_pred_values']
        tf.summary.scalar('offset_mean_abs_error', tf.reduce_mean(tf.abs(
            bin_mean_predictions - tf.layers.flatten(tf.abs(ops['offset_orig_labels_values'])))))
        tf.summary.scalar('offset_max_abs_error', tf.reduce_max(tf.abs(
            bin_mean_predictions - tf.layers.flatten(tf.abs(ops['offset_orig_labels_values'])))))

    merged = tf.summary.merge_all()

    acc_update_ops = top_grasp_acc_summaries(ops, threshold=[0.62, 0.65, 0.68])
    ...
    _, pr_update_op = pr_curve_streaming_func(name='pr_contact_success',
                                              predictions=tf.layers.flatten(
                                                  ops['binary_seg_pred']),
                                              labels=tf.layers.flatten(
                                                  tf.cast(ops['grasp_suc_labels_pc'], tf.bool)),
                                              num_thresholds=201)

    pr_reset_op = tf.variables_initializer(
        tf.get_collection(tf.GraphKeys.METRIC_VARIABLES))
    merged_eval = tf.summary.merge_all()

    session.run(tf.local_variables_initializer())

    summary_ops = {'merged': merged, 'merged_eval': merged_eval, 'pr_update_op': pr_update_op,
                   'auc_update_op': auc_update_op, 'acc_update_ops': acc_update_ops, 'pr_reset_op': pr_reset_op}
    return summary_ops


def build_file_writers(log_dir):
    """
    ...
    # , session.graph) creates large event files!
