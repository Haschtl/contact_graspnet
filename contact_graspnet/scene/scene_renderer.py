...
class SceneRenderer:
    ...
    def _init_camera_renderer(self):
        ...
        if self._intrinsics in ['kinect_azure', 'realsense']:
            ...
            camera = pyrender.PerspectiveCamera(
                yfov=self._fov, aspectRatio=1.0, znear=0.001)  # do not change aspect ratio
            self._camera_node = self._scene.add(
                camera, pose=tra.euler_matrix(np.pi, 0, 0), name='camera')
    ...
    def render_labels(self, full_depth, obj_paths, obj_scales, render_pc=False):
        ...
        obj_names = [path + '_' + str(scale)
                     for path, scale in zip(obj_paths, obj_scales)]
