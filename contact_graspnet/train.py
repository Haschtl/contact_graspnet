...
def train(global_config, log_dir, gripper_name="panda"):
    ...
    num_train_samples = len(contact_infos) - \
        global_config['DATA']['num_test_scenes']
    ...
    with tf.Graph().as_default():
        ...
        loss_ops = load_labels_and_losses(
            grasp_estimator, contact_infos, global_config, True, gripper_name)

        ops.update(loss_ops)
        ops['train_op'] = build_train_op(
            ops['loss'], ops['step'], global_config)
...
def train_one_epoch(session, ops, summary_ops, file_writers, pcreader):
    ...
    for batch_idx in range(pcreader._num_train_samples):
        ...
        loss_log[batch_idx % 10,
                 :] = loss_val, dir_loss, bin_ce_loss, offset_loss, approach_loss, adds_loss, adds_gt2pred_loss

        if (batch_idx+1) % 10 == 0:
            file_writers['train_writer'].add_summary(summary, step)
            f = tuple(np.mean(loss_log, axis=0)) + \
                ((time.time() - get_time) / 10., )
            log_string(
                'total loss: %f \t dir loss: %f \t ce loss: %f \t off loss: %f \t app loss: %f adds loss: %f \t adds_gt2pred loss: %f \t batch time: %f' % f)
            get_time = time.time()
...
def eval_validation_scenes(session, ops, summary_ops, file_writers, pcreader, max_eval_objects=500):
    ...
    for batch_idx in np.arange(min(pcreader._num_test_samples, max_eval_objects)):
        ...
        batch_data, cam_poses, scene_idx = pcreader.get_scene_batch(
            scene_idx=pcreader._num_train_samples + batch_idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str,
                        default=None, help='Grasp data root dir')
    parser.add_argument('--max_epoch', type=int,
                        default=None, help='Epochs to run')
    ...
    os.system('cp {} {}'.format(os.path.join(
        CONTACT_DIR, 'contact_graspnet.py'), ckpt_dir))  # bkp of model def
    # bkp of train procedure
    os.system('cp {} {}'.format(os.path.join(
        CONTACT_DIR, 'train.py'), ckpt_dir))
