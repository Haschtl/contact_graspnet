...
def read_ply_file(ply_file, load_cols=True):
    ...
    with open(ply_file, 'r') as f:
        ...
        ret_dict['xyz'] = np.array(
            [list(map(float, l.strip().split(' '))) for l in verts_lines])
        if load_cols:
            cols_lines = lines[12:12 + 2*verts_num:2]
            ret_dict['xyz_color'] = np.array(
                [list(map(int, l.strip().split(' '))) for l in cols_lines])
...
np.savez(file_name.replace('.ply', '.npz'),
         xyz=ret_dict['xyz'], xyz_color=ret_dict['xyz_color'])
