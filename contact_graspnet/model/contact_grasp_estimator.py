...
class GraspEstimator:
    ...
    def __init__(self, cfg):
        ...
        if 'surface_grasp_logdir_folder' in cfg:
            ...
            self._contact_grasp_cfg = config_utils.load_config(
                cfg['surface_grasp_logdir_folder'], batch_size=1, arg_configs=cfg.arg_configs)
            self._cfg = cfg
        ...
        self._num_input_points = self._contact_grasp_cfg['DATA']['raw_num_points'] if 'raw_num_points' in self._contact_grasp_cfg[
    ...
    def extract_3d_cam_boxes(self, full_pc, pc_segments, min_size=0.3, max_size=0.6):
        ...
        for i in pc_segments:
            pc_segments[i] = reject_median_outliers(
                pc_segments[i], m=0.4, z_only=False)
            ...
            if np.any(pc_segments[i]):
                ...
                size = np.minimum(np.maximum(
                    np.max(obj_extent)*2, min_size), max_size)
                print('Extracted Region Cube Size: ', size)
                partial_pc = full_pc[np.all(full_pc > (
                    obj_center - size/2), axis=1) & np.all(full_pc < (obj_center + size/2), axis=1)]
                if np.any(partial_pc):
                    partial_pc = regularize_pc_point_count(
                        partial_pc, self._contact_grasp_cfg['DATA']['raw_num_points'], use_farthest_point=self._contact_grasp_cfg['DATA']['use_farthest_point'])
    ...
    def predict_grasps(self, session, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        ...
        ), self._num_input_points, return_mean=True, convert_to_internal_coords=convert_cam_coords)
        ...
        if constant_offset:
            offset_pred = np.array([[self._contact_grasp_cfg['DATA']['gripper_width'] -
                                   self._contact_grasp_cfg['TEST']['extra_opening']]*self._contact_grasp_cfg['DATA']['num_point']])
        ...
        selection_idcs = self.select_grasps(pred_points[:, :3], pred_scores,
                                            ...
                                            self._contact_grasp_cfg['TEST']['second_thres'] if 'second_thres' in self._contact_grasp_cfg[
                                                'TEST'] else self._contact_grasp_cfg['TEST']['first_thres'],
        ...
        if 'center_to_tip' in self._contact_grasp_cfg['TEST'] and self._contact_grasp_cfg['TEST']['center_to_tip']:
            pred_grasps_cam[:, :3, 3] -= pred_grasps_cam[:, :3, 2] * \
                (self._contact_grasp_cfg['TEST']['center_to_tip']/2)
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    ...
                    if local_regions and np.any(pred_grasps_cam[k]):
                        print('Generated {} grasps for object {}'.format(
                            len(pred_grasps_cam[k]), k))
                else:
                    print('skipping obj {} since  np.any(pc_segments[k]) {} and np.any(contact_pts[j]) is {}'.format(
    ...
    def select_grasps(self, contact_pts, contact_conf, max_farthest_points=150, num_grasps=200, first_thres=0.25, second_thres=0.2, with_replacement=False):
        ...
        _, center_indexes = farthest_points(contact_pts[conf_idcs_greater_than, :3], np.minimum(
            max_farthest_points, len(conf_idcs_greater_than)), distance_by_translation_point, return_center_indexes=True)

        remaining_confidences = np.setdiff1d(
            np.arange(len(grasp_conf)), conf_idcs_greater_than[center_indexes])
        ...
        if with_replacement:
            ...
            while j < num_grasps and conf_idcs_greater_than.shape[0] > 0:
                selection_idcs.append(
                    conf_idcs_greater_than[j % len(conf_idcs_greater_than)])
                j += 1
            selection_idcs = np.array(selection_idcs)

        else:
            remaining_idcs = sorted_remaining_confidence_idcs[:num_grasps-len(
                conf_idcs_greater_than[center_indexes])]
            remaining_conf_idcs_greater_than = np.nonzero(
                grasp_conf[remaining_idcs] > second_thres)[0]
            selection_idcs = np.union1d(
                conf_idcs_greater_than[center_indexes], remaining_idcs[remaining_conf_idcs_greater_than])
    ...
    def extract_point_clouds(self, depth, K, segmap=None, rgb=None, z_range=[0.2, 1.8], segmap_id=0, skip_border_objects=False, margin_px=5):
        ...
        if K is None:
            raise ValueError(
                'K is required either as argument --K or from the input numpy file')
        ...
        if pc_colors is not None:
            pc_colors = pc_colors[(pc_full[:, 2] < z_range[1]) & (
                pc_full[:, 2] > z_range[0])]
        pc_full = pc_full[(pc_full[:, 2] < z_range[1]) &
                          (pc_full[:, 2] > z_range[0])]
            ...
            for i in obj_instances:
                if skip_border_objects and i != segmap_id:
                    ...
                    if np.any(obj_i_x < margin_px) or np.any(obj_i_x > segmap.shape[1]-margin_px) or np.any(obj_i_y < margin_px) or np.any(obj_i_y > segmap.shape[0]-margin_px):
                        print(
                            'object {} not entirely in image bounds, skipping'.format(i))
                        continue
                inst_mask = segmap == i
                ...
                pc_segments[i] = pc_segment[(pc_segment[:, 2] < z_range[1]) & (
                    pc_segment[:, 2] > z_range[0])]
