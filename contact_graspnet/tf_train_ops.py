...
def load_labels_and_losses(grasp_estimator, contact_infos, global_config, train=True, gripper_name="panda"):
    ...
    tf_pos_contact_points, tf_pos_contact_dirs, tf_pos_contact_approaches, \
        tf_pos_finger_diffs, tf_scene_idcs = load_contact_grasps(
            contact_infos, global_config['DATA'])
    ...
    if train:
        ...
        grasp_dataset = grasp_dataset.apply(
    ...
    if global_config['MODEL']['bin_offsets']:
        ...
        offset_labels_pc = grasp_estimator._model_func.multi_bin_labels(
            offset_labels_pc, global_config['DATA']['labels']['offset_bins'])
    ...
    if global_config['MODEL']['pred_grasps_adds_gt2pred']:
        total_loss += global_config['OPTIMIZER']['adds_gt2pred_loss_weight'] * \
            adds_loss_gt2pred

    tf_bin_values = grasp_estimator._model_func.get_bin_values(global_config)
    ...
    loss_label_ops = {'loss': total_loss,
                      ...
                      'offset_orig_labels_values': orig_offset_labels if global_config['MODEL']['bin_offsets'] else None,
                      'offset_bin_label_values': tf.gather_nd(tf_bin_values, tf.expand_dims(tf.argmax(offset_labels_pc, axis=2), axis=2)) if global_config['MODEL']['bin_offsets'] else None,
                      'grasp_suc_labels_pc': grasp_suc_labels_pc,
                      'approach_labels_pc': approach_labels_pc,
                      'tf_bin_values': tf_bin_values,
...
def build_train_op(total_loss, step, global_config):
    ...
    train_op = optimizer.minimize(
        total_loss, global_step=step, var_list=tf.global_variables())
    if TF2:
...
def load_contact_grasps(contact_list, data_config):
    ...
    for i, c in enumerate(contact_list):
        contact_directions_01 = c['scene_contact_points'][:,
                                                          0, :] - c['scene_contact_points'][:, 1, :]
        all_contact_points = c['scene_contact_points'].reshape(-1, 3)
        all_finger_diffs = np.maximum(np.linalg.norm(
            contact_directions_01, axis=1), np.finfo(np.float32).eps)
        all_contact_directions = np.empty(
            (contact_directions_01.shape[0]*2, contact_directions_01.shape[1],))
        all_contact_directions[0::2] = - \
            contact_directions_01 / all_finger_diffs[:, np.newaxis]
        all_contact_directions[1::2] = contact_directions_01 / \
            all_finger_diffs[:, np.newaxis]
        ...
        if num_pos_contacts > len(all_pos_contact_points)/2:
            ...
            pos_sampled_contact_idcs_replacement = np.random.choice(np.arange(len(
                all_pos_contact_points)), num_pos_contacts*2 - len(all_pos_contact_points), replace=True)
    ...
    with tf.device(device):
        tf_scene_idcs = tf.constant(
            np.arange(0, len(pos_contact_points)), tf.int32)
        tf_pos_contact_points = tf.constant(
            np.array(pos_contact_points), tf.float32)
        tf_pos_contact_dirs = tf.math.l2_normalize(
            tf.constant(np.array(pos_contact_dirs), tf.float32), axis=2)
        tf_pos_finger_diffs = tf.constant(
            np.array(pos_finger_diffs), tf.float32)
        tf_pos_contact_approaches = tf.math.l2_normalize(
            tf.constant(np.array(pos_approach_dirs), tf.float32), axis=2)
